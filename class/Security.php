...
class BoardSecurity
{
    ...
    function update_session($member_id)
    {
        ...
        while ($pref = $this->DB->load_array()) {
            if ($pref['value'] == "t" || $pref['value'] == "true") {
                $_SESSION[$pref['name']] = true;
            } elseif ($pref['value'] == "f" || $pref['value'] == "false") {
                $_SESSION[$pref['name']] = false;
            } else {
    ...
    function form_login($name = false, $pass = false)
    {
      // if logged in and we didn't send a different username flag member_id as session_id
        if (session('id') && ($name == "" && $pass == "")) {
            return session('id');
        } elseif ($login = $this->auth($name, $pass)) {
            return $login;
        } else {
            print ERROR_AUTH;
            return false;
