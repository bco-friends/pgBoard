...
class Form
{
  ...
  // fix this stupid noclose hack
    ...
    function build_select($ob)
    {
        ...
        if ($ob->initial) {
            $output .= "  <option value=\"\">$ob->initial</option>\n";
        }
    ...
    function build_element($ob)
    {
        ...
        switch ($ob->type) {
            case self::FORM_HIDDEN:
                $buff = $this->build_hidden($ob);
                break;
            case self::FORM_TEXT:
                $buff = $this->build_text($ob);
                break;
            case self::FORM_PASSWORD:
                $buff = $this->build_password($ob);
                break;
            case self::FORM_TEXTAREA:
                $buff = $this->build_textarea($ob);
                break;
            case self::FORM_SELECT:
                $buff = $this->build_select($ob);
                break;
            case self::FORM_CHECKBOX:
                $buff = $this->build_checkbox($ob);
                break;
            case self::FORM_RADIO:
                $buff = $this->build_radio($ob);
                break;
            case self::FORM_BUTTON:
                $buff = $this->build_button($ob);
                break;
            case self::FORM_SUBMIT:
                $buff = $this->build_submit($ob);
                break;
        }
        $output = "";
