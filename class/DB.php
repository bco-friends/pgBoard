...
class DB
{
      ...
      // check to see if pg_* functions are available
        if (!function_exists('pg_connect')) {
          ...
          // connect to DB with appropriate verbosity
            ...
            if (!$this->db) {
                if ($this->debug) {
                    trigger_error("Possible malformed connection string, unable to connect to database.\n\n<strong>CONNECTION STRING:</strong>\n$connect");
                }
                return false;
            }
  ...
  // close connection at end of script
    function __destruct()
    {
 /* if(isset($this->db)) pg_close($this->db);*/
    }
  ...
  // execute currently defined sql
    function run_query()
    {
      ...
      // if query failed die gracefully
        if (!$this->res) {
            if ($this->debug) {
                trigger_error("<strong>FAILED</strong> " . $this->get_query() . "\n" . pg_last_error());
            }
            return false;
        } else {
            return $this->res;
  ...
  // load the first result from the first row of executed sql
    function load_result()
    {
      ...
      // if no sql defined die gracefully
        if (!$this->res) {
            if ($this->debug) {
                trigger_error("No query to load result from.");
            }
            return false;
        } else {
  ...
  // output results in XML format -- INCOMPLETE
    function get_xml_results()
    {
      // if no sql defined die gracefully
        if (!$this->res) {
            if ($this->debug) {
                trigger_error("No query to load results from.");
            }
            return false;
        } else {
  ...
  // get the last id from a defined sequence
    function get_last_id($seq)
    {
        ...
        if (!$this->query("SELECT currval('$seq')")) {
            if ($this->debug) {
                trigger_error("Invalid query requesting currval of sequence.");
            }
            return false;
        } else {
            return $this->load_result();
  ...
  // output 'explain analyze' of currently defined sql
    function analyze_query()
    {
        if (!$this->sql) {
            if ($this->debug) {
                trigger_error("No query to analyze.");
            }
            return false;
        } else {
            $temp = $this->sql;
