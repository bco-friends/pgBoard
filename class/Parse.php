...
class BoardParse
{
  ...
  // prepare urls (so hack)
    function prep_url_linktext($href)
    {
        return $this->prep_url(array($href[1]), htmlentities($href[2]));
    }
    function prep_url($href, $link = false)
    {
        $clean = str_replace(array("[url]","[/url]"), "", $href[0]);
        if (substr($clean, 0, 3) == "www") {
            $clean = "http://$clean";
        }
        if (!$link) {
            $link = htmlentities($clean);
        }
        $clean = trim($clean);

        if (!preg_match('|^http(s)?://[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(/.*)?$|i', $clean)) {
            return $href[0];
        } else {
