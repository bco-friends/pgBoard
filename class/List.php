...
class BoardList extends Base
{
    ...
    function prep_data($row)
    {
      ...
      // start shared parsing
        ...
        if (session('id')) {
            if ($data[BoardQuery::LIST_CREATOR_ID] == session('id')) {
                $data['me'] = SPACE . CSS_ME;
            }
            if (in_array($data[BoardQuery::LIST_ID], $this->favorites)) {
      ...
      // start list specific parsing
        switch ($this->type) {
            ...
            case Base::LIST_THREAD_SEARCH:
                if ($data[BoardQuery::LIST_STICKY] == "t") {
                    $data['subject'] = STICKY_TEXT . NON_BREAKING_SPACE . $data['subject'];
                }
                if (session('id')) {
                    if ($data[BoardQuery::LIST_LAST_POSTER_ID] != session('id') && $data[BoardQuery::LIST_DOTFLAG] == "t") {
                        $data['dot'] = LIST_DOT;
                    }
    ...
    function thread($stickies = false) // this stickies flag is crap, find a better way
    {
        ...
        $class = CSS_ODD;
        if (!$this->ajax) {
            print "<div id=\"list_{$id}\" class=\"data\">\n";
        }
        ...
        if (!$this->ajax) {
            ...
            if (!$stickies) {
                print "<ul class=\"{$class}\" id=\"noresults\" style=\"display:$display\">" . NO_RESULTS . "</ul>\n";
            }
            print "</div>\n";
        }
    }
