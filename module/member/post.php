...
{
    global $DB;
    if (session('id') || !REGISTRATION_OPEN || !session('authorized')) {
        return to_index();
    }

    $output = "";
    if ($DB->check("SELECT true FROM member WHERE LOWER(name)=$1", array(strtolower(post('account'))))) {
        $output .= ERROR_MEMBER_NAME_INUSE . "<br/>";
    }

    if (!preg_match(MEMBER_REGEXP, post('account'))) {
        $output .= ERROR_MEMBER_NAME . "<br/>";
    }

    if (preg_match("|^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$|", post('email_signup'))) {
        if (post('email_signup') != post('email_confirm')) {
            $output .= ERROR_MEMBER_EMAIL_NOMATCH . "<br/>\n";
        }
    } else {
        $output .= ERROR_MEMBER_EMAIL . "<br/>\n";
    }

    if ($DB->check("SELECT true FROM member WHERE email_signup=$1", array(post('email_signup')))) {
        $output .= ERROR_MEMBER_EMAIL_INUSE . "<br/>";
    }

    if (!is_numeric(post('postalcode'))) {
        $output .= ERROR_MEMBER_POSTALCODE . "<br/>\n";
    }
    if ($output != "") {
