...
function view_get()
{
  ...
  // set flags for media link
    ...
    if (get('uncollapse')) {
        $flags .= "&uncollapse=true";
    }
    if (session('hidemedia')) {
        if (get('media')) {
            $subtitle .= "<a href=\"" . url() . "$flags\">hide images</a>";
        }
        if (!get('media')) {
            $subtitle .= "<a href=\"" . url() . "$flags\">show images</a>";
        }
    }
    if (!session('hidemedia')) {
        if (!get('media')) {
  ...
  // set flags for collase link
    ...
    if (!session('nocollapse')) {
        if (!get('uncollapse')) {
            $subtitle .= SPACE . ARROW_RIGHT . SPACE . "<a href=\"" . url() . "$flags\">uncollapse</a>";
        }
        if (get('uncollapse')) {
            $subtitle .= SPACE . ARROW_RIGHT . SPACE . "<a href=\"" . url() . "$flags\">collapse</a>";
        }
    }

    if (session('id')) {
        if (!$Core->check_favorite(id())) {
            $subtitle .= SPACE . ARROW_RIGHT . SPACE . "<a href=\"javascript:;\" onclick=\"toggle_favorite(" . id() . ");\"><span id=\"fcmd\">add</span> favorite</a>\n";
        } else {
      ...
      // undot
        if ($Core->check_dotted(id())) {
            $subtitle .= SPACE . ARROW_RIGHT . SPACE . "<a href=\"javascript:;\" onclick=\"undot(" . id() . ");\" id=\"undot\">undot</a>\n";
        }

        if ($Core->check_undotted(id())) {
            $subtitle .= SPACE . ARROW_RIGHT . SPACE . "<a href=\"javascript:;\" onclick=\"redot(" . id() . ");\" id=\"redot\">redot</a>\n";
        }
    ...
    if (get('xml')) {
        ...
        if (get('uncollapse') && !session('nocollapse')) {
            $_SESSION['nocollapse'] = true;
        }
        $View->thread();
        if (get('uncollapse')) {
            unset($_SESSION['nocollapse']);
        }
    }
...
function listfavoritesbymember_get()
{
  ...
  // get info
    ...
    if (!FAVORITES_PUBLIC && $id != session('id')) {
        return to_index();
    }
